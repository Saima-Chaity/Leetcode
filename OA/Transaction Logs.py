...
If same userid appears in the transaction as userid1 and userid2, it should count as one occurrence,
not two.
...
def getFraudIds(logData: List[List[str]], threshold: int) -> List[str]:
    ...
    for item in logData:
        ...
        if splitedLogs[0] == splitedLogs[1]:
            mapping[splitedLogs[0]] = mapping.get(splitedLogs[0], 0) + 1
        else:
            mapping[splitedLogs[0]] = mapping.get(splitedLogs[0], 0) + 1
            mapping[splitedLogs[1]] = mapping.get(splitedLogs[1], 0) + 1

    output = []
    for log, freq in mapping.items():
        if freq >= threshold:
            output.append(log)

    output.sort(key=int, reverse=True)
    return output
