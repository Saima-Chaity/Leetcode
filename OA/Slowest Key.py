...
Keypress for 'b' had a duration of 29 - 9 = 20 (pressed at time 9 right after the release of the
previous character and released at time 29).
Keypress for 'c' had a duration of 49 - 29 = 20 (pressed at time 29 right after the release of the
previous character and released at time 49).
Keypress for 'd' had a duration of 50 - 49 = 1 (pressed at time 49 right after the release of the
previous character and released at time 50).
...
class Solution:
    def slowestKey(self, releaseTimes: List[int], keysPressed: str) -> str:
        ...
        for i in range(len(keysPressed)):
            char = keysPressed[i]
            if i == 0:
                duration[releaseTimes[i]].append(char)
                maxDuration = releaseTimes[i]
            else:
                time = releaseTimes[i] - releaseTimes[i - 1]
                duration[time].append(char)
                if maxDuration < time:
                    maxDuration = time
