'''Winning Sequence
Construct sequence using a specified number of integers within a range. The sequence must be strictly
increasing at first and then strictly decreasing. The goal is to maximize the sequence array elements
...
Output: [9,10,9,8,7]
Explanation:
In this case, [9, 10, 9, 8, 7] is the winning sequence. It maintains the constraints of being first
strictly increasing and then strictly decreasing, and there is no way to have integers in the sequence
that are greater than [9, 10, 9, 8, 7]. So the output is [9, 10, 9, 8, 7].
'''

class Solution:
    def winningSequence(self, num, lowerEnd, upperEnd):


num = 5
lowerEnd = 3
upperEnd = 10
print(Solution.winningSequence((), num, lowerEnd, upperEnd))