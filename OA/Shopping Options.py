...
limited budget in dollars. Given different pricing options for each product, determine
how many options our customer has to buy 1 of each product. You cannot spend more money
...
The customer must buy shoes for 4 dollars since there is only one option. This leaves 6 dollars
to spend on the other 3 items. Combinations of prices paid for jeans, skirts, and tops respectively
that add up to 6 dollars or less are [2, 2, 2], [2, 2, 1], [3, 2, 1], [2, 3, 1]. There are 4 ways
the customer can purchase all 4 items.

Function Description

Complete the getNumberOfOptions function in the editor below. The function must return an integer
which represents the number of options present to buy the four items.
...
class Sol:
    def get_options(self, priceOfJeans, priceOfShoes, priceOfSkirts, priceOfTops, dollars) -> int:
        ...
        print(self.numberOfWays)
        return self.numberOfWays

    def backTrack(self, options, dollars, index):
        if dollars < 0:
            return
        if index >= len(options):
            self.numberOfWays += 1
            return
        for productPrice in options[index]:
            self.backTrack(options, dollars-productPrice, index + 1)
