...
Under the grammar given below, strings can represent a set of lowercase words. Let's use R(expr) to denote
...
class Solution:
    def braceExpansionII(self, expression: str) -> List[str]:
        ...
        mapping = {}
        stack = []
        for index, brace in enumerate(expression):
            if brace == "{":
                stack.append(index)
            elif brace == "}":
                mapping[stack.pop()] = index

        def getResult(low, high):
            answer = [[""]]
            if low + 1 < high:
                i = low
                while i < high:
                    if expression[i] == ",":
                        ...
                        if expression[i] == "{":
                            y = getResult(i + 1, mapping[i])
                            i = mapping[i]
                        else:
                            y = expression[i]
                        answer.append([xx + yy for xx in answer.pop() for yy in y])
                    i += 1
            return sorted({xx for x in answer for xx in x})

        return getResult(0, len(expression))
