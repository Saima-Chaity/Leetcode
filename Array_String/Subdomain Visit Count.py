...
Explanation:
We will visit "google.mail.com" 900 times, "yahoo.com" 50 times, "intel.mail.com" once and "wiki.org" 5 times. For
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for domain in cpdomains:
            ...
            mapping[subDomain] = mapping.get(subDomain, 0) + int(count)
            for i in range(len(subDomain) - 1, -1, -1):
                if subDomain[i] == ".":
                    substring = subDomain[i + 1:]
                    mapping[substring] = mapping.get(substring, 0) + int(count)
