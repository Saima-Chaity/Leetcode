...
def boxBlur(image):
    def calculateSum(start_i, start_j, row, col):
        totalSum = 0
        for i in range(start_i, row + 1):
            for j in range(start_j, col + 1):
                totalSum += image[i][j]
        return totalSum
    ...
    j = 0
    currentCell = []
    while i <= row - 3 and j <= col - 3:
        total = calculateSum(i, j, i + 2, j + 2)
        pixel = total // 9
        currentCell.append(pixel)
        if j < col - 3:
            j += 1
        elif i < row - 3:
            i += 1
            j = 0
            output.append(currentCell)
            currentCell = []
        else:
            output.append(currentCell)
            break
