...
class Solution:
    def maxScore(self, cardPoints: List[int], k: int) -> int:
        ...
        kCardsFromBegining = sum(cardPoints[:k])
        kCardsFromRear = 0
        maxValue = kCardsFromBegining
        for i in range(k - 1, -1, -1):
            kCardsFromBegining -= cardPoints[i]
            kCardsFromRear += cardPoints[len(cardPoints) - (k - i)]
            maxValue = max(maxValue, kCardsFromBegining + kCardsFromRear)
        return maxValue