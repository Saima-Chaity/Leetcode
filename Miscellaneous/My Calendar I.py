...
class MyCalendar:
    ...
    def booking_helper(self, start, end, node):
        if start >= node.end:
            if node.right:
                return self.booking_helper(start, end, node.right)
            else:
                node.right = BST(start, end)
                return True
        elif end <= node.start:
            if node.left:
                return self.booking_helper(start, end, node.left)
            else:
    ...
    def book(self, start: int, end: int) -> bool:
        if not self.root:
            ...
            return True
        return self.booking_helper(start, end, self.root)
