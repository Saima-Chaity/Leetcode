...
class TicTacToe:
    ...
    def move(self, row: int, col: int, player: int) -> int:
        ...
        win = True
        for i in range(0, len(self.matrix)):
            if self.matrix[i][len(self.matrix) - i - 1] != player:
                win = False
                break
...
class TicTacToe:
    ...
    def move(self, row: int, col: int, player: int) -> int:
        ...
        if abs(self.rows[row]) == self.n or abs(self.cols[col]) == self.n or abs(self.diagonal1) == self.n or abs(
                self.diagonal2) == self.n:
            return player
        return 0
